...
from siamg.tracker.base_tracker import SiameseTracker
from siamg.utils.bbox import corner2center

class siamger(SiameseTracker):
    def __init__(self, model):
        super(siamger, self).__init__()
    ...
    def save_cache(self, img, bbox):
        center_cache = np.array([bbox[0]+(bbox[2]-1)/2, bbox[1]+(bbox[3]-1)/2])
        size_cache = np.array([bbox[2], bbox[3]])
        channel_average_cache = np.mean(img, axis=(0, 1))

        w_z = size_cache[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(size_cache)
        h_z = size_cache[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(size_cache)
        s_z = round(np.sqrt(w_z * h_z))

        z_crop_cache = self.get_subwindow(img,center_cache,
                                    cfg.TRACK.EXEMPLAR_SIZE,
                                    s_z, channel_average_cache)

        return (z_crop_cache,size_cache)

    def track_cache(self, img, cache_info):
        """
        args:
            img(np.ndarray): BGR image
            bbox: (x, y, w, h) bbox 
        """
        self.model.template(cache_info[0])
        self.size = cache_info[1]
        w_z = self.size[0] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        h_z = self.size[1] + cfg.TRACK.CONTEXT_AMOUNT * np.sum(self.size)
        s_z = np.sqrt(w_z * h_z)
