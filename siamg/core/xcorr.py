...
def xcorr_pixelwise(x,kernel): #z=kernel 
    ...
    b, c, h, w = x.size() 
    kernel_mat = kernel.view((b, c, -1)).transpose(1, 2).squeeze(0) # (b, hz * wz, c)
    x_mat = x.view((b, c, -1)).squeeze(0)  # (b, c, hx * wx)
    return torch.mm(kernel_mat, x_mat).view((b, -1, h, w))    # (b, hz * wz, hx * wx) --> (b, hz * wz, hx, wx)
