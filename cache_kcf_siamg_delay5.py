...
def set_param(args):
    ...
    if args.video_name.endswith(".mp4") \
        or args.video_name.endswith(".avi"):
        save_video_path = args.video_name.split(video_name)[0] + video_name + '_cache_tracking.mp4'
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    elif args.video_name.endswith(".mkv") :
        fourcc = cv2.VideoWriter_fourcc(*'X264')
        save_video_path = args.video_name.split(video_name)[0] + video_name + '_cache_tracking.mkv'
    ...
    return video_writer,ok, cap, init_rect, frame
...
def main(args):
    ...
    tracker_kcf = kcftracker.KCFTracker(True, True, True)  # hog, fixed_window, multiscale


    video_writer, ok, cap, init_rect, frame = set_param(args)    
    ...
    while cap.isOpened():
        ...
        if outputs_siamg['best_score'] < 0.99:
            if init_flag == False:
                tracker_kcf.init(bbox_siamg, frame)
                init_flag = True
            ...
            init_flag = False
            # if need_fix == False and already_save == False:
            if need_fix == False and cnt_save == 0:
            # if need_fix == False:
